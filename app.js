...
const POSTHOG_KEY = import.meta.env.VITE_PUBLIC_POSTHOG_KEY || import.meta.env.VITE_POSTHOG_KEY || "phc_jOSqRCRkMpP79C8gA4f99k0V4HXncArsezkrqfocID6";
...
function analyticsBaseProps(extra = {}) {
  const region = currentRegion();
  const active = activePokemon();
  return {
    region_id: region?.id || "",
    region_name: region?.name || "",
    wild_zone_id: state.game.selectedWildZoneId || "",
    in_wild: Boolean(state.game.inWild),
    badges_count: state.game.badges.length,
    money: state.game.money,
    team_size: state.game.team.length,
    active_pokemon: active?.speciesName || "",
    active_level: active?.level || 0,
    total_wins: state.game.totalWins,
    ...extra,
  };
}

function analyticsCapture(eventName, properties = {}) {
  if (!analyticsState.enabled) return;
  try {
    posthog.capture(eventName, analyticsBaseProps(properties));
  } catch (error) {
    console.warn(`PostHog capture failed: ${eventName}`, error);
  }
}

function analyticsSessionEnd(reason = "page_leave") {
  if (!analyticsState.enabled || analyticsState.sessionEnded) return;
  analyticsState.sessionEnded = true;
  const playTimeSec = Math.max(1, Math.round((Date.now() - analyticsState.sessionStartMs) / 1000));
  analyticsCapture("session_end", {
    reason,
    play_time_sec: playTimeSec,
    gym_badges: state.game.badges.length,
    tournament_rounds_cleared: state.game.tournamentIndex,
  });
  try {
    posthog.flush?.();
  } catch {
...
function initAnalytics() {
  ...
  try {
    analyticsState.sessionStartMs = Date.now();
    analyticsState.sessionEnded = false;
    ...
    if (["localhost", "127.0.0.1"].includes(window.location.hostname)) {
      posthog.debug(true);
      console.info("[Analytics] PostHog initialized on localhost.");
    }
    analyticsState.enabled = true;
    analyticsCapture("session_started", {
      start_screen: "screenStart",
      has_saved_trainer: Boolean(localStorage.getItem(SAVE_KEY)),
    });
  } catch (error) {
...
function attemptCapture(enemy) {
  ...
  const chanceRounded = Number(chance.toFixed(3));
  const hpRatioRounded = Number(hpRatio.toFixed(3));
  const speciesName = enemy.speciesName || String(enemy.name || "").toLowerCase();
  ...
  if (caught) {
    ...
    if (state.game.team.length < 6) {
      ...
      analyticsCapture("pokemon_captured", {
        pokemon: speciesName,
        capture_target: "party",
        capture_chance: chanceRounded,
        enemy_hp_ratio: hpRatioRounded,
      });
    ...
    analyticsCapture("pokemon_capture_failed", {
      pokemon: speciesName,
      capture_chance: chanceRounded,
      enemy_hp_ratio: hpRatioRounded,
    });
...
function closeBattle(victory, captured = false, meta = {}) {
  ...
  const outcome = captured ? "captured" : victory ? "win" : "loss";
  analyticsCapture("battle_finished", {
    outcome,
    battle_mode: mode,
    battle_label: state.battle.label,
    captured_pokemon: captured ? captureName.toLowerCase() : "",
    money_delta: moneyDelta,
    exp_gained: expInfo.expGained,
    level_from: expInfo.fromLevel,
    level_to: expInfo.toLevel,
    enemy_team_size: enemyTotal,
    enemy_defeated: enemyDefeated,
    player_team_ready: playerReady,
  });

  if (mode === "gym" && !captured) {
    analyticsCapture("gym_battle_finished", {
      victory,
      battle_label: state.battle.label,
      badge_earned: badgeEarned || "",
      gym_index: state.game.gymIndex,
      badges_count: state.game.badges.length,
    });
  }
...
async function startBattle(mode, enemyNames, label, options = {}) {
  ...
  analyticsCapture("battle_started", {
    battle_mode: mode,
    battle_label: label,
    enemy_team_size: enemyNames.length,
    enemy_names: enemyNames.join(","),
    can_run: state.battle.canRun,
    level_base: options.levelBase || 5,
  });
...
async function startWildEncounter(options = {}) {
  if (!state.game.inWild) {
    ...
    analyticsCapture("wild_entered", {
      region_id: currentRegion().id,
      zone_id: state.game.selectedWildZoneId,
    });
    renderHub();
  }
  ...
  if (typeof options.trainerIndex === "number" && region.wildTrainers?.[options.trainerIndex]) {
    ...
    analyticsCapture("wild_encounter_requested", {
      encounter_type: "trainer",
      trainer_name: trainer.name,
      trainer_index: options.trainerIndex,
      zone_id: state.game.selectedWildZoneId,
    });
  ...
  if (typeof options.wildName === "string" && pool.includes(options.wildName)) {
    ...
    analyticsCapture("wild_encounter_requested", {
      encounter_type: "selected_wild",
      wild_pokemon: options.wildName,
  ...
  analyticsCapture("wild_encounter_requested", {
    encounter_type: "random_wild",
    wild_pokemon: wildName,
    level_estimate: level,
    zone_id: state.game.selectedWildZoneId,
  });
...
function buyItem(key) {
  ...
  analyticsCapture("shop_purchase", {
    item_key: key,
    item_name: item.name,
    item_price: item.price,
    owned_after_purchase: state.game.inventory[key],
  });
  persistGame();
  renderHub();
  showShop(true);
...
async function showStarterOptions() {
  ...
  starters.forEach((starter) => {
    ...
    card.querySelector("button").addEventListener("click", async () => {
      ...
      analyticsCapture("game_started", {
        start_type: "new",
        avatar_id: state.game.avatar,
        trainer_name_length: state.game.playerName.length,
      });
      analyticsCapture("starter_chosen", {
        starter: fighter.speciesName,
        starter_level: fighter.level,
        starter_max_hp: fighter.stats.maxHp,
      });
      saveGame();
...
function bindEvents() {
  ...
  qs("beginJourneyBtn").addEventListener("click", async () => {
    ...
    analyticsCapture("trainer_profile_created", {
      avatar_id: state.ui.selectedAvatar,
      trainer_name_length: name.length,
    });
  ...
  window.addEventListener("pagehide", () => analyticsSessionEnd("page_hide"));
  window.addEventListener("beforeunload", () => analyticsSessionEnd("before_unload"));
}
